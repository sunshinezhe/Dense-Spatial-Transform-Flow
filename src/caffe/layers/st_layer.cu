...
void SpatialTransformerLayer<Dtype>::Forward_gpu(
	...
	SpatialTransformerForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
	      CAFFE_CUDA_NUM_THREADS>>>(nthreads, N, C, output_H_, output_W_, H, W, theta, U, V);
}
...
__global__ void SpatialTransformerBackwardGPU_dTheta(const int nthreads, int C,
	...
	CUDA_KERNEL_LOOP(index, nthreads) {
		...
		m = floor(x); n = floor(y); 
		if(m >= 0 && m < H && n >= 0 && n < W) {
			delta_dpx -= (1 - (y - n)) * U[m * W + n] * dV ;
			delta_dpy -= (1 - (x - m)) * U[m * W + n] * dV ;
		}
		
		// left-top neighbor
		m = floor(x); n = floor(y) + 1; 
		if(m >= 0 && m < H && n >= 0 && n < W) {
			delta_dpx -= (1 - (n - y)) * U[m * W + n] * dV ;
			delta_dpy += (1 - (x - m)) * U[m * W + n] * dV ;
		}
		...
		int idx =  s * output_W_ *C+ t*C;
		
		dTheta_tmp_diff[ i * (2 * output_H_ * output_W_ * C) + idx+j] = delta_dpy;
		dTheta_tmp_diff[ i * (2 * output_H_ * output_W_ * C) + idx+j+output_H_ * output_W_ * C] = delta_dpx;
...
void SpatialTransformerLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
	...
	SpatialTransformerBackwardGPU_dTheta<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
			CAFFE_CUDA_NUM_THREADS>>>(nthreads, C, output_H_, output_W_, H, W, theta,
	...
	caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, bottom[1]->count(), 1, C, 
	...
	*/
	...
	if(to_compute_dU_) {
		...
		SpatialTransformerBackwardGPU_dU<Dtype><<<CAFFE_GET_BLOCKS(nthreads),
			CAFFE_CUDA_NUM_THREADS>>>(nthreads, C, W, H, output_H_, output_W_, theta, dV, dU);
	}
}
